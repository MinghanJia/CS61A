...
def swine_align(player_score, opponent_score):
    ...
    if min_num == 0:
        return False
    GCD = 1
    for i in range(1, opponent_score+1):
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0 :
            first_roll = strategy0(score0, score1)
            score0 += take_turn(first_roll, score1, dice)
            if extra_turn(score0, score1):
                who = who
            else:
                who = other(who)
        else:
            second_roll = strategy1(score1, score0)
            score1 += take_turn(second_roll, score0, dice)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def increasing(score0, score1):
        running_high_temp = running_high
        last_score_temp = last_score
        if who == 0:
            if score0 - last_score > running_high:
                print("%d point(s)! The most yet for Player %d" % (score0 - last_score, who))
                running_high_temp = score0 - last_score
            last_score_temp = score0
        else:
            if score1 - last_score > running_high:
                print("%d point(s)! The most yet for Player %d" % (score1 - last_score, who))
                running_high_temp = score1 - last_score
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    average=[]
    for i in range(1, 11):
        average_dic = make_averaged(roll_dice)
        average.append(average_dic(i, dice))
    max_average = max(average)
    return average.index(max_average) + 1
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_ = score + free_bacon(opponent_score)
    return 0 if extra_turn(score_, opponent_score) or bacon_strategy(score,opponent_score, cutoff, num_rolls) == 0 else num_rolls  # Replace this statement
    # END PROBLEM 11
